...
--
  ...
  - use the recent `qgis_process` shell command
...
- **qgisprocess** (Dewey Dunnington et al.): direct interface to `qgis_process` shell command
...
qgis_algorithms() |>
  ...
  filter(str_detect(algorithm, "point.*line")) |> 
  knitr::kable()
```
